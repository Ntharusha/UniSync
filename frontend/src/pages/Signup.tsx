...
import { Calendar, User, Lock, ArrowRight, AlertCircle, Briefcase, Mail } from 'lucide-react';
...
export default function Signup({ onLogin }: { onLogin: (user: UserType) => void }) {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    // Custom check: Vavuniya university institutional email domain check (optional warning or constraint)
    if (!email.endsWith('@vau.ac.lk')) {
      setError('Please use your University of Vavuniya institutional email address (@vau.ac.lk).');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(err.error || 'Registration failed. Please check your credentials and try again.');
      }
    } catch (err) {
      ...
      >
        <div className="bg-white/80 backdrop-blur-xl py-10 px-4 shadow-2xl sm:rounded-3xl sm:px-10 border border-white/20">
          ...
          <form onSubmit={handleSignup} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-black text-gray-400 uppercase tracking-widest px-1">Role</label>
              <div className="grid grid-cols-2 gap-4">
                <button
                  ...
                  className={`py-3 rounded-2xl font-bold border-2 transition-all ${
                    role === 'student'
                      ? 'border-vau-maroon bg-vau-maroon/5 text-vau-maroon'
          ...
          <p className="mt-8 text-center text-sm text-gray-600">
            ...
            <Link to="/login" className="font-bold text-vau-maroon hover:underline">
              Sign In
            </Link>
          </p>
          ...
          <div className="mt-8 p-6 bg-vau-gold/10 rounded-2xl">
            <p className="text-xs font-bold text-vau-maroon/80 text-center leading-relaxed">
              Use your institutional credentials ending with @vau.ac.lk to sign up. 
              If you face issues, reach out to the Department administrator.
            </p>
          </div>
        </div>
      </motion.div>
